...
def create_database():
    ...
    if not c.fetchone():  # Se não encontrar a tabela 'tasks', cria a tabela
        c.execute('''CREATE TABLE tasks
                     (task_name text, description text, status text, limit_date date, priority text)''')
...
def view_tasks(order_by=None):
    ...
    c = conn.cursor()  # Cria um cursor para executar comandos SQL
    if order_by:
        c.execute(f"SELECT task_name, description, status, limit_date, priority FROM tasks ORDER BY {order_by}")  # Seleciona e ordena as tasks pelo campo especificado
    else:
        c.execute("SELECT task_name, description, status, limit_date, priority FROM tasks")  # Seleciona todas as tasks
...
def main():
    ...
    if operation == "Add Task":
        ...
        if selected_task:
            ...
            limit_date = st.text_area("Description", value=current_description)
            priority = st.selectbox("Prioridade", ["Low", "Mid", "High"], index=["Low", "Mid", "High"].index(current_priority))
        ...
        order_by = st.selectbox("Order by", ["task_name", "description", "status", "limit_date", "priority"])  # Caixa de seleção para escolher o campo de ordenação
